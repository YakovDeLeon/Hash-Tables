#ifndef _HOPSCOTCH_HASH_TABLE_2_
#define _HOPSCOTCH_HASH_TABLE_2_
#include<type_traits>
#include<cstdint>
#include<functional>
#include<exception>
...
// hash table for incremenatl hopscotch set
template<bool NeedStoreHash>
struct hop_2_base_bucket
{
...
template <class T, unsigned int NB, bool StoreHash, class H, class EQ>
class incremental_rehash_hopscotch_set;
...
>
class hopscotch_hash_table_2 : private Hash, private KeyEqual
{
    static_assert(NeighborhoodSize < 62,
                  "Max size of NeighborhoodSize is 61");
    static constexpr std::size_t NB_RESERVED_BITS = 2;
    static constexpr bool NeedStoreHash = StoreHash && !(std::is_arithmetic<T>::value);
    friend class incremental_rehash_hopscotch_set<T, NeighborhoodSize, StoreHash, Hash, KeyEqual>;
public:
    using value_type = T;
    ...
    struct bucket : hop_2_base_bucket<NeedStoreHash>
    {
        storage data;
...
public:
    hopscotch_hash_table_2() = default; //: hopscotch_hash_table_2(4){}
    explicit hopscotch_hash_table_2(unsigned char log2_bucket_cnt_)
    ...
    {}
    hopscotch_hash_table_2(const hopscotch_hash_table_2 &) = delete;
    hopscotch_hash_table_2 &operator=(const hopscotch_hash_table_2 &) = delete;
    ~hopscotch_hash_table_2() { destroy(); }
    ...
    void swap(hopscotch_hash_table_2 &t) noexcept
    {
...
private:
    ...
    size_type erase_impl(const T& v, size_t hash)
    {
