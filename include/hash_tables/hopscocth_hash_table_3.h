#ifndef _HOPSCOTCH_HASH_TABLE_3_
#define _HOPSCOTCH_HASH_TABLE_3_
#include<type_traits>
#include<cstdint>
#include<functional>
#include<exception>
...
// hash table for non-trivial data types
...
>
class hopscotch_hash_table_3 : private Hash, private KeyEqual
{
    static_assert(NeighborhoodSize < 31,
                  "Max size of NeighborhoodSize is 30");
    static constexpr std::size_t NB_RESERVED_BITS = 2;
    friend class incremental_rehash_hopscotch_set<T, NeighborhoodSize, true, Hash, KeyEqual>;
public:
    using value_type = T;
...
public:
    hopscotch_hash_table_3() = default; //: hopscotch_hash_table_2(4){}
    explicit hopscotch_hash_table_3(unsigned char log2_bucket_cnt_)
        : table(allocate_table(size_type(1) << log2_bucket_cnt_))
        , data_storage(log2_bucket_cnt_ ? (size_type(1) << log2_bucket_cnt_) : 0)
        , capacity(log2_bucket_cnt_ ? (size_type(1) << log2_bucket_cnt_) : 0)
        , log2_bucket_cnt(log2_bucket_cnt_)
    {}
    hopscotch_hash_table_3(const hopscotch_hash_table_3 &) = delete;
    hopscotch_hash_table_3 &operator=(const hopscotch_hash_table_3 &) = delete;
    ~hopscotch_hash_table_3() { destroy(); }
   void create(unsigned log2_bucket_cnt_)
    ...
    void swap(hopscotch_hash_table_3 &t) noexcept
    {
        ...
        if(empty_idx < bucket_count())
        {
            ...
            { // Empty bucket is in range of NeighborhoodSize, use it
                if(empty_idx - orig_idx < NeighborhoodSize) 
                {
                    ...
                    table[empty_idx].set_value_of_empty_bucket(bucket_hash, &(*it));
                    table[orig_idx].
